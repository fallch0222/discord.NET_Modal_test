...
namespace DiscordBot
{
    class Program
    {
        ...
        private async Task OnClientMessage(SocketMessage arg)
        {
            ...
            builder.WithButton("질문하기", "spawnModal");
            ...
            client.ButtonExecuted += spawnModal;
            
            
            
            await context.Channel.SendMessageAsync("Here you are!",components: builder.Build()); //수신된 명령어를 다시 보낸다.
        ...
        public async Task spawnModal(SocketMessageComponent soket)
        {
            var mb = new ModalBuilder();
            mb.WithTitle("질문 생성기");
            mb.WithCustomId("mt");
            mb.AddTextInput("질문 내용", "TI1", TextInputStyle.Short, "질문 내용을 입력해주세요!", 2, 100, true);
            


            await soket.RespondWithModalAsync(mb.Build());
            ...
            client.ModalSubmitted += async modal =>
            {
                ...
                string testInput = components.First(x => x.CustomId == "TI1").Value;
                // Build the message to send.
                
                ulong channelID = Convert.ToUInt64(945645079224275024); 
                var textChannel = (SocketTextChannel)client.GetChannel(channelID);

                await textChannel.CreateThreadAsync(modal.User.Username + " 님의 " + testInput +
                                                    " 에 대한 질문!");
